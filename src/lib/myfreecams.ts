...
export const parseMyFreeCamsListing = (html: string, sourceUrl: URL): DiscoveredChannel[] => {
  ...
  for (const match of normalized.matchAll(/<a\b([^>]*title\s*=\s*["']Enter Chat Room of\s+[^"']+["'][^>]*)>([\s\S]*?)<\/a>/gi)) {
    const title = match[1].match(/title\s*=\s*["']Enter Chat Room of\s+([^"']+)["']/i)?.[1]?.trim();
    if (!title || !/^[a-z0-9_.-]{2,80}$/i.test(title) || seen.has(title.toLowerCase())) continue;
    const body = match[2];
    const viewers = Number(body.match(/(\d[\d,.]*)\s*(?:viewers?|watching|users?)/i)?.[1]?.replace(/[,.]/g, '') ?? 0);
    const imageValue = body.match(/(?:data-src|data-original|data-thumb|src)\s*=\s*["']([^"']+)["']/i)?.[1];
    let logo = '';
    ...
    channels.push({ name: title, location: `${Number.isFinite(viewers) ? viewers : 0} viewers · myfreecams.com`, url: `https://mfc.im/${encodeURIComponent(title)}/chat`, logo, viewers: Number.isFinite(viewers) ? viewers : 0 });
  }
